import React, { Component } from 'react';
import './FourthPage.scss'

class FourthPage extends Component {
    render() {
        return (
<>
<section className="headerrr">
        <div className="containerry">
            <div className="toppy">
                <div className="logooo" data-aos="flip-right">
                    <span>Zip <br/>Lyne</span>
                </div>
                <div className="sideMenuuu">
                    <span className="captionnn">Lorem ipsum dolor.</span>
                    <ul id="fourthUl">
                        <li id="fourthli"><a id="fourthA" href=".">lorem</a></li>
                        <li id="fourthli"><a id="fourthA" href=".">ipsum</a></li>
                        <li id="fourthli"><a id="fourthA" href=".">dolor</a></li>
                    </ul>
                </div>
            </div>
            <div className="line111">Lorem, ipsum dolor sit amet, consectetur.</div>
            <div className="">
                <p className="line222">Lorem ipsum dolor sit amet consectetur adipisicing elit. Maxime repudiandae est vero</p>
            </div>
            <div className="fourthbtn">
                <button id="fourthbutton">Lorem ipsum</button>
            </div>
        </div>
    </section>
    <section>
        <div className="section1-1">
            <div className="card">
                <h4>lorem</h4>
                <div className="imgBox">
                    <img data-aos="flip-up" href="." src="https://dribbble.s3.amazonaws.com/users/322/screenshots/872485/coldchase.jpg" alt="."/>
                </div>
                <h3>Lorem ipsum dolor</h3>
                <p>Lorem ipsum, dolor sit amet consect adipisicing elit.
                     Aliquam blanditiis dolor eum minus, labore</p>
            </div>
            <div className="card">
                <h4>lorem</h4>
                <div className="imgBox">
                    <img data-aos="flip-up" href="." src="https://dribbble.s3.amazonaws.com/users/322/screenshots/980517/icehut_sm.jpg" alt="."/>
                </div>
                <h3>Lorem ipsum dolor</h3>
                <p>Lorem ipsum, dolor sit amet consect adipisicing elit.
                     Aliquam blanditiis dolor eum minus, labore</p>
            </div>
            <div className="card">
                <h4>lorem</h4>
                <div className="imgBox">
                    <img data-aos="flip-up" href="." src="https://dribbble.s3.amazonaws.com/users/322/screenshots/943660/hq_sm.jpg" alt="."/>
                </div>
                <h3>Lorem ipsum dolor</h3>
                <p>Lorem ipsum, dolor sit amet consect adipisicing elit.
                     Aliquam blanditiis dolor eum minus, labore</p>
            </div>
        </div>
    </section>
    <footer>
        <div className="footer">
            <div className="copright">
                <p>©Lorem ipsum dolor</p>
            </div>
            <div className="footerNav">
                <ul id="fourthUl">
                    <li id="fourthli"><a id="fourthA" href=".">lorem</a></li>
                    <li id="fourthli"><a id="fourthA" href=".">ipsum</a></li>
                    <li id="fourthli"><a id="fourthA" href=".">dolor</a></li>
                </ul>
            </div>
        </div>
    </footer>




</>
        );
    }
}

export default FourthPage;